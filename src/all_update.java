...
public class all_update extends JFrame{

	JLabel l1 = new JLabel("<html>변경 하고 싶은 조건식(속성이름 관계연산자 값 형태)을 입력해주시길 바랍니다.<br>"+ " 단, set 부터 모든 조건식을 입력해주셔야 됩니다. 예) set movie_name = \"좀비영화2\" where movie_no = 2 ");
	
	JLabel l2 = new JLabel("<html>테이블 속성이름은 아래 콤보박스를 클릭하면 GUI 화면에 정보가 나옵니다.<br>"+ " 테이블속성이름에 해당하는 테이블에 대하여 변경을 진행 합니다.");
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e){
				...
				if (tf.getText().equals("")) {
					 JOptionPane.showMessageDialog(null, "변경하고 싶으면 조건식을 입력해주세요.","메시지",JOptionPane.ERROR_MESSAGE);
				}else if(table_name.getSelectedIndex()==-1){
					 JOptionPane.showMessageDialog(null, "콤보박스에서 변경하고 싶은 테이블 속성을 먼저 확인 해주세요.","메시지",JOptionPane.ERROR_MESSAGE);
		               ...
		               mgr.sql = "select * from "+t[k]+" where "+
				               tf.getText().split("where")[1]+";";
		               switch (k) {
		              ...
		              // System.out.println(mgr.sql);
		               if (list.size()>0) {
							...
							mgr.sql = "update "+t[k]+" "+tf.getText()+";";
							mgr.update();
							JOptionPane.showMessageDialog(null, "변경이 완료되었습니다.","메시지",JOptionPane.INFORMATION_MESSAGE);
