...
public class movie_reservation extends JFrame{
	...
	JComboBox<String> screen_date = new JComboBox<String>(); //빈콤보박스
	...
	private void set() {
		...
		mgr.sql = "select DISTINCT screen_date from screen_date where movie_no = "+movie_num+";";
		//System.out.println(mgr.sql);
		list11 = mgr.jo1();
		for (int i = 0; i < list11.size(); i++) {
			bean11 = list11.get(i);
			screen_date.addItem(bean11.getA());
		}
		...
		//예약된좌석 비활성화 
		//영화번호 기준으로 상영일정 번호 가져오기 그리고 상영관번호 기준으로 
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(flag==0) {
					...
					mgr.sql = "select screen_date_no from screen_date where movie_no ="+movie_num+" and screen_no ="+screen_num.getSelectedItem()+" and screen_time = '"+screen_time.getSelectedItem()+"';";
	...
	private void combo2() {
		...
		mgr.sql = "SELECT DISTINCT screen_time FROM screen_date where movie_no = "+movie_num+" and screen_date = '"+screen_date.getSelectedItem()+"' and screen_no = "+screen_num.getSelectedItem()+";";
	...
	private void seat() {
		...
		mgr.sql = "select screen_date_no from screen_date where movie_no ="+movie_num+" and screen_no = "+screen_num.getSelectedItem()+";";
		// and screen_date = '"+screen_date.getSelectedItem()+"'
		list9  = mgr.jo1();
		bean9 = list9.get(0);
		//System.out.println(mgr.sql);
	...
	private void combo() { //상영관 번호에따라 좌석수 변경되는곳
		...
		mgr.sql = "SELECT DISTINCT screen_no FROM screen_date where movie_no = "+movie_num+" and screen_date ='"+screen_date.getSelectedItem()+"';";
		list2 = mgr.jo1();
