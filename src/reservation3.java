...
public class reservation3 extends JFrame {
    ...
    static String sdate = "2021-01-01";
	...
	public reservation3() {
		...
		set();
//이영화에 해당하는 모든 상영일정을 가져온다.
		//SELECT m.movie_no, m.movie_name, m.movie_time, m.movie_class, m.movie_supervision, m.movie_actor, m.movie_genre, s.screen_date, m.movie_price, m.movie_intro  FROM movie m ,screen_date s  where m.movie_no = 1 and  m.movie_no = s.movie_no group by s.screen_date order by m.movie_no;

		mgr.sql = "SELECT m.movie_no, m.movie_name, m.movie_time, m.movie_class, m.movie_supervision, m.movie_actor, m.movie_genre,s.screen_date, m.movie_price, m.movie_intro  FROM movie m inner join screen_date s on m.movie_no = s.movie_no "
				+ "where m.movie_no = "+member_main.mv+" group by s.screen_date order by m.movie_no;";
		...
		menu1.addActionListener(new ActionListener() {  //예약 누를때 
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					movie_reservation2.movie_num =Integer.parseInt(table1.getValueAt(row, 0).toString()); //영화번호 넘겨줘
				...
				if(date.compareTo(date2) >= 0) {
					...
					try {
						...
						//movie no를 기준으로 
						sdate = table1.getValueAt(row, 7).toString();
						movie_reservation2.screen_date.setText(sdate);
